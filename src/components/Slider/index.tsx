    ...
    <Grid padding="16px 0 16px 0">
      ...
      <Grid width="100%" templateColumns="16px auto 16px" columnGap="4px">
        <Grid
          ...
          onClick={() => {
            if (state.prevIndex - 8 > 0) {
              setState({
                prevIndex: state.prevIndex - 8,
                nextIndex: state.nextIndex - 8
              });
            } else {
        ...
        {type === "artist" && (
          ...
          >
            {array.length > 0 &&
              array.map((item: IArtist, index: number) => {
                return (
                        ...
                        <Image
                          width={
                            toggleSidebar.state.show
                              ? "calc(calc(100vw - 150px - 40px - 40px - 112px) / 8)"
                              : "calc(calc(100vw - 40px - 40px - 112px) / 8)"
                          }
                          height={
                            toggleSidebar.state.show
                              ? "calc(calc(100vw - 150px - 40px - 40px  - 112px) / 8)"
        ...
        {type === "music" && (
          ...
          >
            {array.length > 0 &&
              array.map((item: IAlbum, index: number) => {
                return (
                  index >= state.prevIndex &&
                  index <= state.nextIndex && (
