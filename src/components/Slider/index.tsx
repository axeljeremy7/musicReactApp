...
const Slider: React.FC<ISlider> = ({artists , title, borderRadius = 'unset'}) => {
    ...
    return (
        ...
        <Grid width='100%' templateColumns='16px auto 16px' columnGap='4px'>
            ...
            <Grid templateColumns='repeat(8, 1fr)' columnGap='16px' justifyContent='center'>
                {artists.length > 0 &&  artists.map((item : IArtist, index: number) => 
                    {
            ...
            <Grid width='16px' height='16px' margin='-16px 0 0 0'
                onClick={() => 
                    {
                        if(state.nextIndex + 8 < artists.length){
                            setState({prevIndex: state.prevIndex + 8, nextIndex: state.nextIndex + 8})
                        }else{
                            setState({prevIndex: 0, nextIndex: 7})
                        }
